...
class TestRunsPrepare(BaseTest):
    def setUp(self):
        super().setUp()
        patcher = patch(
            "core.orchestration.scheduler.manager.SchedulingManager.runs_start"
        )
        patcher.start()
        self.addCleanup(patcher.stop)

        patcher = patch(
            "core.orchestration.scheduler.manager.SchedulingManager.runs_stop"
        )
        patcher.start()
        self.addCleanup(patcher.stop)

    @mock.patch("core.orchestration.scheduler.manager.SchedulingManager._resolve")
    ...
    @mock.patch("core.common.workers.send")
    @mock.patch("core.orchestration.scheduler.manager.SchedulingManager._resolve")
